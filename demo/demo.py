# demo.py
# Copyright (c) 2025 Tobias Karusseit
# Licensed under the MIT License. See LICENSE file in the project root for full license information.

# data optained from open flight data
# https://openflights.org/data.php

from graph.graph import RouteGraph
import os

if __name__ == "__main__":

    path = os.path.dirname(os.path.abspath(__file__))
    # initialize the graph
    graph = RouteGraph(
        maxDistance=50,
        transportModes={"airport": "fly", },
        dataPaths={"airport": os.path.join(path, "data", "fullDataset.csv")},
        compressed=False,
    )
    # build the graph
    graph.build()
    # save the graph
    graph.save(os.path.join(path, "data", "graph"), compressed=False)
    # set start and end points
    start = 60.866699,-162.272996 # Atmautluak Airport
    end = 60.872747,-162.5247 #Kasigluk Airport

    start_hub = graph.findClosestHub(["airport"], *start) # find the hubs
    end_hub = graph.findClosestHub(["airport"], *end)
    # find the shortest path
    route = graph.find_shortest_path(start_id=start_hub.id, end_id=end_hub.id, allowed_modes=["fly"], verbose=True)
    print(route.flatPath)