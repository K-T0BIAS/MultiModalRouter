...
class TestRouteGraphInit(unittest.TestCase):
    def test_init_with_no_data_paths(self):
        graph = RouteGraph(maxDistance=50, transportModes={}, compressed=False, extraMetricsKeys=[], drivingEnabled=True)
    ...
    def test_init_with_data_paths(self):
        ...
        graph = RouteGraph(maxDistance=50, transportModes={'airport': 'fly', 'shippingport': 'shipping'}, dataPaths=data_paths, compressed=False, extraMetricsKeys=[], drivingEnabled=True)
        self.assertEqual(graph.compressed, False)
        self.assertEqual(graph.extraMetricsKeys, [])
        self.assertEqual(graph.drivingEnabled, True)
    ...
    def test_init_with_extra_metrics_keys(self):
        ...
        graph = RouteGraph(maxDistance=50, transportModes={'airport': 'fly'}, dataPaths={'airport': 'airports.csv'}, compressed=False, extraMetricsKeys=extra_metrics_keys, drivingEnabled=True)
        self.assertEqual(graph.compressed, False)
